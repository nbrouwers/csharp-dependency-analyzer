...
- **FR-7.2**: Each in-scope type shall be imported as a Neo4j node whose label is the type label (`class`, `interface`, `struct`, or `enum`) and whose properties match the CSV export columns exactly: `id` (16-character lowercase hex string — first 8 bytes of SHA-256(`{fqn}:{typeLabel}`) — unique and deterministic, serves as the MERGE key), `name` (simple unqualified name), `type` (same value as the node label), `label` (same value as the node label), `file` (source file path, relativized to the project root using forward slashes), `startLine` (1-based), `endLine` (1-based), `accessibility` (e.g. `"public"`), `fullyqualifiedname` (original .NET FQN), `resolved` (`true` for all in-scope types). Types referenced in code but not resolvable to an in-scope type (unresolved references) that share a namespace root with at least one in-scope type shall also be imported as nodes with `resolved = false` and `type = "class"`.
- **FR-7.3**: The import shall use `MERGE` statements keyed on the `id` property to be idempotent — re-running the import on the same database shall not produce duplicate nodes or relationships.
- **FR-7.4**: Dependency edges shall be mapped to typed Neo4j relationships mirroring the CSV export relationship types: `basecompoundref {startLine, endLine}` for inheritance and interface-implementation edges; `ref {startLine, endLine}` for all other usage edges. The `startLine` and `endLine` properties shall reflect the call-site location (best-effort; 0 if unavailable).
...
- **FR-7.8**: Before importing any data, the tool shall clear the target database by executing `MATCH (n) DETACH DELETE n`. This ensures the database contains exactly the exported graph — no stale data from prior runs is retained.
